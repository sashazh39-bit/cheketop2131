#!/usr/bin/env python3
"""Патч 08-03-26_00-00 9.pdf:
- Плательщик: Александр Евгеньевич Ж. -> Евгений Александрович Е.
- Сумма: 10 ₽ -> 10 000 ₽
zlib level 6, метаданные сохраняются.

Использование: python3 patch_08_03_9_10_to_10000.py input.pdf output.pdf
"""
import re
import sys
import zlib
...
# 10 ₽ -> 10 000 ₽ (1=0014, 0=0013, space=0003, ₽=04@)
# Порядок: 1, 0, пробел, 0, 0, 0, пробел, ₽
OLD_AMOUNT_10 = b'(\x00\x14)-11.11111 (\x00\x13)-11.11111 (\x00\x03)-11.11111 (\x04@)'
NEW_AMOUNT_10000 = b'(\x00\x14)-11.11111 (\x00\x13)-11.11111 (\x00\x03)-11.11111 (\x00\x13)-11.11111 (\x00\x13)-11.11111 (\x00\x13)-11.11111 (\x00\x03)-11.11111 (\x04@)'
OLD_TM_AMOUNT = b"1 0 0 1 231.52501 72.37499 Tm"
OLD_TM_AMOUNT_ALT = b"1 0 0 1 210 72.37499 Tm"
NEW_TM_AMOUNT = b"1 0 0 1 204.21451 72.37499 Tm"  # по /Widths: W4=25.1 W8=52.4, правый край 256.62


def main():
    if len(sys.argv) < 3:
        print("Использование: python3 patch_08_03_9_10_to_10000.py input.pdf output.pdf")
        sys.exit(1)
    ...
    for m in re.finditer(rb"<<(.*?)/Length\s+(\d+)(.*?)>>\s*stream\r?\n", data, re.DOTALL):
        ...
        if OLD_AMOUNT_10 in new_dec:
            new_dec = new_dec.replace(OLD_AMOUNT_10, NEW_AMOUNT_10000)
            changed = True
        if NEW_AMOUNT_10000 in new_dec:
            for old_tm in (OLD_TM_AMOUNT, OLD_TM_AMOUNT_ALT):
                if old_tm in new_dec:
