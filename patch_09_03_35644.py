#!/usr/bin/env python3
"""Патч 09-03-26_00-00.pdf:
- Сумма: 10 ₽ -> 35 644 ₽
- Плательщик: Александр Евгеньевич Ж. -> Евгений Александрович Е.
- /ID обновляется под новый контент (снижает риск «признаков подделки»)

Использование: python3 patch_09_03_35644.py input.pdf output.pdf
"""
import re
import sys
import hashlib
...
# 10 ₽ -> 35 644 ₽ (3=0016, 5=0018, space=0003, 6=0019, 4=0017, 4=0017, space=0003, ₽=04@)
OLD_AMOUNT = b'(\x00\x14)-11.11111 (\x00\x13)-11.11111 (\x00\x03)-11.11111 (\x04@)'
NEW_AMOUNT = (
    b'(\x00\x16)-11.11111 (\x00\x18)-11.11111 (\x00\x03)-11.11111 '
    b'(\x00\x19)-11.11111 (\x00\x17)-11.11111 (\x00\x17)-11.11111 (\x00\x03)-11.11111 (\x04@)'
)
OLD_TM_AMOUNT = b"1 0 0 1 231.52501 72.37499 Tm"
NEW_TM_AMOUNT = b"1 0 0 1 202.60801 72.37499 Tm"


def main():
    if len(sys.argv) < 3:
        print("Использование: python3 patch_09_03_35644.py input.pdf output.pdf")
        sys.exit(1)
    ...
    for m in re.finditer(rb"<<(.*?)/Length\s+(\d+)(.*?)>>\s*stream\r?\n", data, re.DOTALL):
        ...
        if changed:
            ...
            # Обновление /ID — чтобы изменённый документ не светился «подделкой»
            ...
            print("[OK] Евгений Александрович Е., 35 644 ₽")
            break
    else:
